...
# 2. Clean the 'furnished' column: trim whitespace and standardize case (optional, but good practice)
df_feature_prepare$furnished <- trimws(df_feature_prepare$furnished) # Remove leading/trailing spaces
# df_feature_prepare$furnished <- tolower(df_feature_prepare$furnished) # Convert to lowercase if needed
...
df_feature_prepare$furnished <- df_feature_prepare$furnished %>%
  recode(
    ...
    "Not Furnished" = 3,
    .default = NA_real_ # Explicitly handle anything unmatched
  ) %>%
  as.numeric()

# Check unique values after recoding
print("Unique values in 'furnished' after recoding:")
print(unique(df_feature_prepare$furnished))


# --- Diagnosing and Cleaning 'property_type' column ---
# 1. Check unique values before recoding
print("Unique values in 'property_type' before recoding:")
print(unique(df_feature_prepare$property_type))

# 2. Clean the 'property_type' column: trim whitespace and standardize case (if applicable)
df_feature_prepare$property_type <- trimws(df_feature_prepare$property_type) # Remove leading/trailing spaces
# df_feature_prepare$property_type <- tolower(df_feature_prepare$property_type) # Convert to lowercase if needed
...
df_feature_prepare$property_type <- df_feature_prepare$property_type %>%
  recode(
    "Condominium" = 1,
    "Service Residence" = 0,
    .default = NA_real_ # Essential to catch unmatched values
  ) %>%
  as.numeric()

# Check unique values after recoding
print("Unique values in 'property_type' after recoding:")
print(unique(df_feature_prepare$property_type))
...
# Make sure to work on the df_combined_facilities object
...
keep_columns <- c("rooms", "parking", "bathroom", "near_public_transport",
                  "monthly_rent", "size",
                  "Popular_location_distance", "property_type",
                  "furnished", "property_age", "number_facilities", "location")

# Filter out unwanted columns from the last dataframe where you performed transformations
...
# Assuming df_model is your complete dataset (df_feature_prepare after initial feature engineering)
# And 'split' variable is defined (e.g., split = 0.8)

# --- 1. Initial Data Splitting ---
train_index_regression <- createDataPartition(df_model$monthly_rent, p = split, list = FALSE)

# Create copies for modification, select out target variable
X_train_regression <- df_model[train_index_regression, ] %>% select(-monthly_rent)
X_test_regression <- df_model[-train_index_regression, ] %>% select(-monthly_rent)
...
# --- 2. Standardize 'monthly_rent' (Target Variable y) ---
...
# --- 3. Standardize Numeric Features in X ('size', 'Popular_location_distance') ---
...
# --- 4. Encode 'location' (Label Encoding) ---
# Ensure 'location' is a factor in your original df_model for consistent level ordering
# Or convert it here for train/test sets explicitly
...
# Get all unique levels from the *training* data to ensure consistent encoding
# This is crucial! Only train set levels define the mapping.
...
# Handle potential NAs in test set if new locations appear
# Decide on a default value:
# - NA_integer_ (if your model can handle NAs, e.g., some tree-based models)
# - 0 (if 0 is not used by any existing category and makes sense as a "default/unseen" category)
# - The mean/median of other encoded locations (less common for label encoding)
# Let's use 0 as a common placeholder for unseen categories in label encoding.
default_location_value <- 0
X_test_regression$location_encoded[is.na(X_test_regression$location_encoded)] <- default_location_value
X_test_classification$location_encoded[is.na(X_test_classification$location_encoded)] <- default_location_value


# --- 5. Remove Original (Unscaled/Unencoded) Columns ---
X_train_regression <- X_train_regression %>%
  select(-size, -Popular_location_distance, -location) # Use comma for multiple columns to remove
...
print("Head of X_train_regression (after all transformations):")
head(X_train_regression)
print("Head of X_test_regression (after all transformations):")
head(X_test_regression)
```
...
```{r}
# Assuming lm_model has been trained as in your code snippet

# 1. Access the model's coefficients
# For an "lm" method in train, the actual linear model object is stored within lm_model$finalModel
...
# Remove the intercept term, as it's not a feature importance
coefficients_df <- coefficients_df[-1, , drop = FALSE] # remove the (Intercept) row

# Calculate absolute coefficients for importance ranking
coefficients_df$Abs_Coefficient <- abs(coefficients_df$Coefficient)

# Order by absolute coefficient value (descending)
feature_importance <- coefficients_df[order(coefficients_df$Abs_Coefficient, decreasing = TRUE), ]

# Print the top N most important features
print("Feature Importance (Linear Regression - based on absolute coefficients):")
print(head(feature_importance, 10)) # Adjust 10 to see more or fewer top features

# Optional: Visualize feature importance (using ggplot2)
library(ggplot2)

# Prepare data for plotting (top N features)
top_n <- 15 # Adjust as needed
plot_data <- head(feature_importance, top_n)
plot_data$Feature <- rownames(plot_data)

ggplot(plot_data, aes(x = reorder(Feature, Abs_Coefficient), y = Abs_Coefficient)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  coord_flip() + # Makes the labels readable
  labs(title = paste0("Top ", top_n, " Feature Importance (Linear Regression)"),
       x = "Feature",
...
|-------------------|----------|------------|----------|---------|
| Linear Regression | 1.060901 | 0.09617472 | 1.12551  | 0.00252 |
| Decision Tree     | 1.062292 | 0.08849119 | 1.128465 | NA      |
| XGBoost           | 1.065795 | 0.08985116 | 1.135919 | 0.00008 |
| LightGBM          | 1.071493 | 0.102214   | 1.148098 | 0.00001 |
| Neural Network    | 1.062122 | 0.09297507 | 1.128103 | NA      |
...
| Kappa               | 0.1352  |
| No Info Rate        | 0.5082  |
| Class 1 Sensitivity | 0.7974  |
| Class 2 Sensitivity | 0.0000  |
| Class 3 Sensitivity | \~0.360 |
