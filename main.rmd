...
#Encode 2 to 0 for property_type

df_model$property_type_encoded[df_model$property_type_encoded==2] = 0
...
# Ensure lightgbm is loaded
library(lightgbm)

# Convert features to matrix and labels to numeric (LightGBM needs labels starting from 0)
lgb_train_data <- lgb.Dataset(data = as.matrix(X_train_classification), 
                              label = as.numeric(y_train_classification) - 1)

# Set LightGBM parameters
params <- list(
  objective = "multiclass",
  metric = "multi_logloss",
  num_class = length(unique(y_train_classification)),
  learning_rate = 0.05,
  num_leaves = 31
)
...
pred_probs <- predict(lgb_model, as.matrix(X_test_classification))

# Convert probabilities to predicted class index
pred_matrix <- matrix(pred_probs, ncol = length(unique(y_train_classification)), byrow = TRUE)
pred_labels <- max.col(pred_matrix)

# Convert to factor with original levels
predicted_class <- factor(pred_labels, labels = levels(y_train_classification))

# Evaluate model performance
confusionMatrix(predicted_class, y_test_classification)
